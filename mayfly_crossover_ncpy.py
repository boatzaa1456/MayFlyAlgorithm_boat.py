...
def mayfly(name_path_input, num_gen, pop_size, *parameters):
    ...
    # ---------------------- ส่วนของ Mayfly ตัวเมีย ---------------------- #
        ...
        ranked_female_new_cur_sols = []
        ranked_female_population = sorted([female_evaluations_new_pos[sol] for sol in range(half_pop_size)], key=lambda x: x[1])
        for rank in ranked_female_population:
            ranked_female_new_cur_sols.append(sum(rank[2], []))

        # Mate the mayflies
    # สมมติว่า cxPartialyMatched ไม่เปลี่ยนแปลง inputs
        offspring_1 = []
        offspring_2 = []
        for off in range(half_pop_size):
            male = ranked_male_new_cur_sols[off]
            female = ranked_female_new_cur_sols[off]

            # ทำการคัดลอกเพียงครั้งเดียว
            male_copy = male[:]
            female_copy = female[:]

            # คำนวณ offspring
            offspring_1_result, offspring_2_result = cxPartialyMatched(male_copy, female_copy)
            offspring_1.append(offspring_1_result)
            offspring_2.append(offspring_2_result)
