...
def nuptial_dance(velocity_dict, change_percent = 0.5):
    for arc in velocity_dict:
        for key in arc:
            # Directly increase or decrease based on random choice
            if random.choice([-1, 1]) > 0:
                # Increase speed
                arc[key] *= (1 + change_percent)
            else:
                # Decrease speed
                arc[key] *= (1 - change_percent)
...
def mayfly(name_path_input, num_gen, pop_size, *parameters):
    ...
    female_each_gen = []
    female_attractor = []
    count_nuptial_dance = 0
    count_random_flight = 0
    count_male_normal_update = 0
    count_female_normal_update = 0
    ...
    for gen in range(num_gen):
        ...
        for sol in range(half_pop_size):
            ...
            if male_current_value <= male_pbest_value[sol]:
                ...
                if male_pbest_value[sol] <= gbest_value:
                    count_nuptial_dance += 1
                    gbest_value = male_pbest_value[sol]
                    gbest_sol = male_pbest_sol[sol]
                    gbest_arc_sol_cut = male_pbest_arc_sols_cut[sol]
                    mnew_velocity= nuptial_dance(male_velocity_dict[sol][:])
                    mcut_set = creat_cut_set(mnew_velocity, alpha)
            ...
            if not better_than_gbest:
                ...
                mnew_pos = sol_position_update(mcut_set, male_arc_sols_cut[sol], sub_E_list, male_cur_sols[sol][0],
                                               male_pbest_sol[sol][0], gbest_sol[0])[0]
                mevaluation = evaluate_all_sols(mnew_pos, df_item_pool, heavy_item_set, name_path_input)
        ...
        # ---------------------- Female Mayfly Section ----------------------
            ...
            if not better_than_gbest:
                ...
                female_each_gen.append(fevaluation[1])
                count_female_normal_update += 1

        gbest_each_gen.append(gbest_value)
        progress_percent = (gen + 1) / num_gen * 100
    #     print(f'Progress: {progress_percent:.2f}%')
    # print("----" * 50)
    # print(f"Number of Nuptial Dance: {count_nuptial_dance}")
    # print(f"Number of Random Flight: {count_random_flight}")
    # print(f"Number of Male Normal Update : {count_male_normal_update}")
    # print(f"Number of Female Normal Update : {count_female_normal_update}")
    # print("----" * 50)
...
# gbest_each_gen, male_each_gen, female_each_gen = mayfly(name_path_input, num_gen, pop_size, a1, a2, a3, gmax, gmin,
#                                 alpha, random_seed)
