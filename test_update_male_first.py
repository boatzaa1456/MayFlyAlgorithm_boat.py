...
def mayfly(name_path_input, num_gen, pop_size, *parameters):
    ...
    for gen in range(num_gen):
        for sol in range(half_pop_size):
            ...
            if male_current_value <= male_pbest_value[sol]:
                ...
                if male_pbest_value[sol] <= gbest_value:
                    ...
                    if male_cur_sols_value[sol] <= gbest_value:
                        male_cur_sols[sol] = nuptial_dance(male_cur_sols[sol][:], gbest_sol)
        ...
        for sol in range(half_pop_size):
            ...
            male_each_gen.append((male_evaluations_new_pos[sol][1]))
        ...
        for sol in range(half_pop_size):
            ...
            if female_cur_sols_value[sol] <= male_cur_sols_value[sol]:
                female_cur_sols[sol] = random_flight(female_cur_sols[sol][:])
        ...
        for sol in range(half_pop_size):
            ...
            female_each_gen.append(female_evaluations_new_pos[sol][1])
    ...
    print("----" * 50)
    # return min(gbest_each_gen)
    return gbest_each_gen,male_each_gen,female_each_gen
...
gbest_each_gen,male_each_gen,female_each_gen = mayfly(name_path_input, num_gen, pop_size, a1, a2, a3, gmax, gmin, alpha, random_seed)
...
# คำนวณค่าเฉลี่ยของคำตอบในแต่ละรอบ
average_male = [np.mean(gen) for gen in male_each_gen]
average_female = [np.mean(gen) for gen in female_each_gen]

# คำนวณ standard deviation ของคำตอบในแต่ละรอบ
std_deviation_male = [np.std(gen) for gen in male_each_gen]
std_deviation_female = [np.std(gen) for gen in female_each_gen]

# กำหนดรอบการทำงาน (generations)
rounds = list(range(1, len(male_each_gen) + 1))

plt.figure(figsize=(12, 6))

# ตรวจสอบและแก้ไขการใช้ marker และ linestyle
plt.plot(rounds, gbest_each_gen, label='Best Global Solution (gbest)', marker='_', markersize=4, color='red', linewidth=2)

plt.plot(rounds, average_male, '-o', color='blue', label='Average Male Value', markersize=4, linewidth=2)
plt.plot(rounds, average_female, '-x', color='green', label='Average Female Value', markersize=5, linewidth=2)

plt.fill_between(rounds, np.array(average_male) - np.array(std_deviation_male),
                 np.array(average_male) + np.array(std_deviation_male), color='blue', alpha=0.2, label='Std.Deviation of Male Value')
plt.fill_between(rounds, np.array(average_female) - np.array(std_deviation_female),
                 np.array(average_female) + np.array(std_deviation_female), color='green', alpha=0.2, label='Std.Deviation of Female Value')

plt.xlabel(f"Time Taken: {int(hours)} hours, {int(minutes)} minutes, and {seconds:.2f} seconds")
plt.ylabel('Values')
plt.title(f'{name_path_input} - {pop_size} Population Size - {num_gen} Generations - Seed {random_seed} - Tadiness: {gbest_value}')
