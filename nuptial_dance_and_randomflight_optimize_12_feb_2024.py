...
def nuptial_dance(solution, attractor):
    ...
    num_swaps = 1 if size <= 20 else int(size / 10)  # ตัวอย่าง: ขนาดกลางและใหญ่จะมีการสลับมากขึ้น
    ...
    for gen in range(num_gen):
        ...
        for sol in range(half_pop_size):
            ...
            if female_current_value <= female_pbest_value[sol]:
                ...
                female_attractor = female_current_sol[:]
...
rounds = np.arange(1, len(male_each_gen) + 1)
...
plt.fill_between(rounds, np.array(average_male) - np.array(std_deviation_male), np.array(average_male) + np.array(std_deviation_male), color='blue', alpha=0.2, label='Std.Deviation of Male Value')
plt.fill_between(rounds, np.array(average_female) - np.array(std_deviation_female), np.array(average_female) + np.array(std_deviation_female), color='green', alpha=0.2, label='Std.Deviation of Female Value')
