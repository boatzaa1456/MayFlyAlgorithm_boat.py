...
def nuptial_dance(gbest_value,male_value,male_positon):
    for sol in male_value:
        if male_value[sol] in gbest_value:
            random.shuffle(male_positon[sol])
    return male_positon
    ...
    #---------------------- ส่วนของ Mayfly ตัวผู้ ----------------------
        ...
        for sol in range(half_pop_size):
            if male_cur_sols_value[sol] == gbest_value:
                male_cur_sols_shuffle = nuptial_dance(gbest_value, male_cur_sols_value, male_cur_sols.copy())
                for cur_sol in male_cur_sols:
                    male_cur_sols[cur_sol] = male_cur_sols_shuffle[cur_sol]
