...
def coef_times_position_with_gaussian_test_no_random(arc_first, arc_second, a, beta):
    # Convert arc_second to a list of sets for fast lookups
    arc_second_sets = [set(arcs) for arcs in arc_second]

    coef_times_position_dict = []

    for arcs in arc_first:
        arcs_difference = set(arcs) - set.union(*arc_second_sets)
        rp = len(arcs_difference)
        c_value = a * math.exp(-beta * rp ** 2)

        # Construct the dict for this item using dict comprehension
        velocities = {arc: min(1, round(c_value, 4)) for arc in arcs_difference}
    ...
    for gen in range(num_gen):
        ...
        for sol in range(half_pop_size):
            ...
            if male_cur_sols[sol] == gbest_sol:
                ...
                male_velocity_dict[sol] = copy.deepcopy(init_velocity_sol(male_arc_sols_cut[sol]))
        ...
        for sol in range(half_pop_size):
            ...
            if female_cur_sols[sol] <= gbest_sol:
