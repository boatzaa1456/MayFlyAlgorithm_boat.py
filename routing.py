...
def precedence_constrained_routing(item_list ,*info):
    ...
    for i in item_each_aisle_test[item]:
        cLb_cur += ((int(abs(cur_pos - i) / 2 + 0.5) * rack_y) if i % 2 == 0 else (int(abs(
            cur_pos - i) / 2) * rack_y)) + aisle_x
        #print(f'Lb *** In aisle {travel_aisle[0]}, from location {cur_pos} to location {i}: {cLb_cur}')
        cur_pos = i
    cLa_cur = cLb_cur
    ...
    if len(item_each_aisle_test) == 1:
        ...
        #print(
            #f'Lb *** In aisle {travel_aisle[0]} and from down, from location {cur_pos} '
            #f'to the bottom location {slot[travel_aisle[0]][1]}: {cLb_cur}')
        cLb_cur += rack_y / 2
        cLb_cur += enter_aisle
        #print(f'Lb *** In aisle {cur_aisle} , for the bottom location to the front cross aisle: {cLb_cur}')
        #print(f'Lb *** In aisle {cur_aisle} the distance for Lb_cur: {cLb_cur}')
    ...
    for i in travel_aisle[1:]:
        ...
        cur_aisle_prv = cur_aisle
        cur_aisle = i
        #print(f'cur_aisle_prv = {cur_aisle_prv}')

        # Calculate Lb_cur
        cLb_cur += int(abs(slot[cur_aisle_prv][1] - cur_pos) / 2) * rack_y
        #print(
            #f'Lb *** In aisle {cur_aisle_prv} , from location {cur_pos} '
            #f'to the bottom location {slot[cur_aisle_prv][1]}: {cLb_cur}')
        cLb_cur += rack_y / 2
        cLb_cur += enter_aisle
        #print(f'Lb *** In aisle {cur_aisle_prv} , for the bottom location to the front cross aisle: {cLb_cur}')
        #print(f'Lb *** In aisle {cur_aisle_prv} the distance for Lb_cur: {cLb_cur}')
        ...
        #print(
            #f'La *** In aisle {cur_aisle_prv}, from location {cur_pos} to the top location {slot[cur_aisle_prv][0]}: {cLa_cur}')
        cLa_cur += enter_aisle
        cLa_cur += rack_y / 2
        #print(f'La *** In aisle {cur_aisle_prv}, from the top location to the back cross aisle: {cLa_cur}')
        #print(f'La *** In aisle {cur_aisle_prv}, the distance for La_cur: {cLa_cur}')
        #print(f'++++++++++++++++++++++++++')
        a = 'a' + str(i)
        b = 'b' + str(i)
        ...
        #print(f'Aisle {cur_aisle} = {distance_between_aisle}')
        #''' Calculate Lb_cur '''
        #print('***Lb_cur***')
        cLb_cur = cLb_prv
        cLb_cur += distance_between_aisle
        #print(f'Lb*** In aisle {cur_aisle} at the front cross aisle: {cLb_cur} ')
        cLb_cur += enter_aisle
        cLb_cur += rack_y / 2
        cur_pos = slot[i][1]
        #print_tr(f'Lb*** In aisle {cur_aisle} from start the bottom location {cur_pos}: {cLb_cur}')
        for j in item_each_aisle_test[item]:
            cLb_cur += ((int(abs(cur_pos - j) / 2 + 0.5) * rack_y) if j % 2 == 0 else (int(abs(
                cur_pos - j) / 2) * rack_y)) + aisle_x
            #print(f'Lb*** In aisle {cur_aisle} from location {cur_pos} to location {j}: {cLb_cur}')
            cur_pos = j
        #print('***La_cur***')
        cLa_cur = cLa_prv
        cLa_cur += distance_between_aisle
        #print(f'La*** In aisle {cur_aisle} from location {cur_pos} to the back cross aisle: {cLa_cur} ')
        cLa_cur += enter_aisle
        cLa_cur += rack_y / 2
        cur_pos = slot[i][0]
        #print_tr(f'La*** In aisle {cur_aisle} from start the top location {cur_pos}: {cLa_cur}')
        for j in item_each_aisle_test[item]:
            cLa_cur += ((int(abs(cur_pos - j) / 2) * rack_y) if j % 2 == 0 else (int(abs(
                cur_pos - j) / 2 + 0.5) * rack_y)) + aisle_x
            #print(f'La*** In aisle {cur_aisle} from location {cur_pos} to location {j}: {cLa_cur}')
            cur_pos = j

        if cLa_cur < cLb_cur:
            list_routing.append(a)
            #print(f' ++ cLa_prv = {cLa_cur} ++ < cLb_prv = {cLb_cur} ')
            cLa_cur = cLa_cur
            cLb_cur = cLa_cur
        ...
        #print(f'list_routing = {list_routing}, cLa_prv = {cLa_cur}, cLb_prv = {cLb_cur} ')
        item += 1
        #section_divider(100)
    ...
    #print(f'Lb*** In aisle {cur_aisle_prv} for location {cur_pos} '
         #f'to the last location {slot[cur_aisle][1]} :{total_distance}')
    total_distance += enter_aisle
    total_distance += rack_y / 2
    #print(f'Lb*** In aisle {cur_pos} at the front cross-aisle : {total_distance}')
    distance = int(cur_aisle) * cross_aisle
    total_distance += int(cur_aisle) * cross_aisle
    #total_distance += int(cur_aisle) * cross_aisle
    #print(f'In aisle 0, from the front cross aisle of aisle {cur_aisle} to aisle 0 = {distance} : {total_distance}')
    total_distance += distance_y
    #print(f'From the front cross aisle of aisle 0 to the depot: {total_distance}')
    list_routing.append('depot')
    picker_route = list_routing
    #lines = f"The picker's route is {picker_route} with the total distance of {total_distance}"
    #print(tr.fill(lines, width=60))
    #section_divider(100)
