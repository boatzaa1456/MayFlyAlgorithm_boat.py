...
def cxPartialyMatchedLevyBrownianAdaptive(ind1, ind2, gbest):
    ...
    # Choose random points for PMX crossover
    ...
    # Decide to explore or exploit based on similarity to gbest
    should_explore = (ind1 != gbest and ind2 != gbest)

    # Adaptive MPA motion based on exploration need
    for ind in [ind1, ind2]:
        if should_explore:
            # More aggressive changes for exploration
            if random.random() < 0.25:  # Reduced probability for Levy flight
                i, j = sorted(random.sample(range(len(ind)), 2))
                ind[i:j] = reversed(ind[i:j])
        else:
            # More subtle changes for exploitation
        ...
        for sol in range(half_pop_size):
            if female_cur_sols_value[sol] <= female_pbest_value[sol]:
                ...
                if female_pbest_value[sol] <= gbest_value:
                    ...
                    # if female_cur_sols_value[sol] in male_cur_sols_value[sol]:
...
mayfly(name_path_input, 50, 20, 0.5, 10, 0.2, 0.1)
