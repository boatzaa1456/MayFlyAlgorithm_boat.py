...
def mayfly(name_path_input, num_gen, pop_size, *parameters):
    ...
    for gen in range(num_gen):
        ...
        for sol in range(half_pop_size):
            ...
            if male_cur_sols_value[sol] <= male_pbest_value[sol]:
                ...
                if male_pbest_value[sol] <= gbest_value:
                    ...
                    gbest_arc_sol_cut = copy.deepcopy(male_pbest_arc_sols_cut[sol])  # Consider shallow copy if performance is an issue

            # Update velocities and positions
            male_coef_velocity.append(coef_times_velocity(gravity, male_velocity_dict[sol]))
        ...
        for sol in range(half_pop_size):
            ...
            if female_cur_sols_value[sol] <= female_pbest_value[sol]:
                ...
                if female_pbest_value[sol] <= gbest_value:
                    ...
                    gbest_arc_sol_cut = copy.deepcopy(female_pbest_arc_sols_cut[sol])  # Consider shallow copy if performance is an issue

            # Update velocities and positions
            female_coef_velocity.append(coef_times_velocity(gravity, female_velocity_dict[sol]))
            female_pbest_diff.append(coef_times_position(a1, position_minus_position(female_pbest_arc_sols_cut[sol],female_arc_sols_cut[sol])))
            female_gbest_diff.append(coef_times_position(a2, position_minus_position(gbest_arc_sol_cut, female_arc_sols_cut[sol])))
            female_added_pbest_gbest.append(add_velocity(female_gbest_diff[sol], female_pbest_diff[sol]))
            female_added_velocity.append(add_velocity(female_coef_velocity[sol], female_added_pbest_gbest[sol]))
        ...
        # Process female evaluations after updating all females
        for female_pos in female_new_pos:
            evaluation = evaluate_all_sols(female_pos, df_item_pool, name_path_input)
            female_new_cur_sols.append(sum(evaluation[2], []))
            female_new_cur_sols_value.append(evaluation[1])

        # Update female new solutions
        for sol in range(half_pop_size):
    ...
    plt.ylabel('Fitness')
    plt.title(f'Num_gen = {num_gen}, pop_size = {pop_size}, a1 = 0.5, a2 = 10, beta = 0.2, gravity = 0.1')
    plt.show()
