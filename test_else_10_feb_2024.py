...
def random_flight(solution):
    ...
    if len(solution) < 2:
        return solution  # หรืออาจจะ handle อย่างอื่นตามที่คุณต้องการ
...
def split_and_shuffle_offspring(offsprings, indpb, mutation_func):
    # แยกคู่ของ offspring ออกจากกัน
    separated_offsprings = [off for pair in offsprings for off in pair]

    # ใช้ mutation กับแต่ละ offspring
    for individual in separated_offsprings:
        mutation_func(individual, indpb)

    # สุ่มเรียงลำดับของ offspring ที่แยกออกมา
    random.shuffle(separated_offsprings)

    # แบ่งสุ่มเป็นชายและหญิงโดยให้มีจำนวนเท่ากัน
    half_size = len(separated_offsprings) // 2
    offspring_male = separated_offsprings[:half_size]
    offspring_female = separated_offsprings[half_size:half_size * 2]  # ให้แน่ใจว่าเอาเท่ากับ male

    return offspring_male, offspring_female

def gravity_calculation(gmax, gmin, gen, num_gen):
    gravity = gmax - ((gmax - gmin) * gen / num_gen)
    return gravity

def merge_and_rank_solutions(set1, set2, max_size):
    # รวมสองชุดของคำตอบ
    combined_solutions = set1 + set2

    # เรียงลำดับตามค่าประสิทธิภาพ (ตำแหน่งที่ 1)
    ranked_solutions = sorted(combined_solutions, key=lambda x: x[1])

    # ตัดส่วนที่เกินออก เพื่อให้ได้ขนาดลิสต์ตามที่กำหนด
    trimmed_solutions = ranked_solutions[:max_size]

    return trimmed_solutions


def mayfly(name_path_input, num_gen, pop_size, *parameters):
    a1, a2, a3, gmax,gmin, alpha,mutation_rate ,seed = parameters
    random.seed(seed)
    ...
    male_pbest_arc_sols_cut, female_pbest_arc_sols_cut = [[] for _ in range(half_pop_size)], [[] for _ in
                                                                                              range(half_pop_size)]
    ...
    for gen in range(num_gen):
        ...
        for sol in range(half_pop_size):
            ...
            if female_cur_sols_value[sol] <= male_cur_sols_value[sol]:
                random_flight(female_cur_sols[sol][:])
                female_current_arc_sol = sol_from_list_to_arc(female_cur_sols[sol])
                female_cur_arc_sols[sol] = [tuple(item) for item in female_current_arc_sol]
        ...
        for sol in range(half_pop_size):
            ...
            if female_evaluations_new_pos[sol][1] <= female_cur_sols_value[sol]:
                female_cur_sols_value[sol] = female_evaluations_new_pos[sol][1]
                female_cur_sols[sol] = female_new_pos[sol]
        ...
        for sol in range(half_pop_size):
            ...
            if male_cur_sols_value[sol] <= gbest_value:  # ทดสอบเปรียบเทียบตัวเมีย กับ gbest
                nuptial_dance(male_cur_sols[sol][:], gbest_sol)
                male_current_arc_sol = sol_from_list_to_arc(male_cur_sols[sol])
                male_cur_arc_sols[sol] = [tuple(item) for item in male_current_arc_sol]
        ...
        for sol in range(half_pop_size):
            ...
            coef_male.append(coef_pbest_diff)
            ...
            if male_evaluations_new_pos[sol][1] <= male_cur_sols_value[sol]:
                male_cur_sols_value[sol] = male_evaluations_new_pos[sol][1]
                male_cur_sols[sol] = male_new_pos[sol]
                male_velocity_dict[sol] = male_velocity_check_incon[sol]

        ranked_male_eval_sols = []
        ranked_female_eval_sols = []
        ranked_male_cur_sols = []
        ranked_female_cur_sols = []
        offspring = []
        offspring_male = []
        offspring_female = []
        eva_offspring_male = []
        eva_offspring_female = []
        cxmale_new_eval_sols = []
        cxmale_new_cur_sols = []
        cxfemale_new_eval_sols = []
        cxfemale_new_cur_sols = []

        for sol in range(half_pop_size):
            rank_male = sorted(male_evaluations_new_pos, key=lambda x: x[1])
            rank_female = sorted(female_evaluations_new_pos, key=lambda x: x[1])
            ranked_male_eval_sols.append(rank_male[sol])
            ranked_female_eval_sols.append(rank_female[sol])
            ranked_male_cur_sols.append(extract_and_flatten(ranked_male_eval_sols[sol]))
            ranked_female_cur_sols.append(extract_and_flatten(ranked_female_eval_sols[sol]))
            offspring.append(cxPartialyMatched(ranked_male_cur_sols[sol],ranked_female_cur_sols[sol]))
            offspring_male,offspring_female = split_and_shuffle_offspring(offspring,mutation_rate,mutShuffleIndexes)
            eva_offspring_male.append(evaluate_all_sols(offspring_male[sol], df_item_pool,heavy_item_set,name_path_input))
            eva_offspring_female.append(evaluate_all_sols(offspring_female[sol], df_item_pool, heavy_item_set, name_path_input))
            cxmale_new_eval_sols = merge_and_rank_solutions(ranked_male_eval_sols,eva_offspring_male, half_pop_size)
            cxfemale_new_eval_sols = merge_and_rank_solutions(ranked_female_eval_sols, eva_offspring_female, half_pop_size)
            cxmale_new_cur_sols.append(extract_and_flatten(cxmale_new_eval_sols[sol]))
            cxfemale_new_cur_sols.append(extract_and_flatten(cxfemale_new_eval_sols[sol]))


            if cxmale_new_eval_sols[sol][1] <= male_cur_sols_value[sol]:
                male_cur_sols_value[sol] = cxmale_new_eval_sols[sol][1]
                male_cur_sols[sol] = cxmale_new_cur_sols[sol]
                male_velocity_dict[sol] = male_velocity_check_incon[sol]
            if cxfemale_new_eval_sols[sol][1] <= female_cur_sols_value[sol]:
                female_cur_sols_value[sol] = cxfemale_new_eval_sols[sol][1]
                female_cur_sols[sol] = cxfemale_new_cur_sols[sol]
                female_velocity_dict[sol] = female_velocity_check_incon[sol]
...
gbest_per_gen = mayfly(name_path_input, num_gen, pop_size, a1, a2, a3, gmax,gmin, alpha,mutation_rate, random_seed)
print(gbest_per_gen)
# End the timer
end_time = time.time()
...
plt.title(
    f'(Mayfly Algorithm - {name_path_input}) - {pop_size} Population Size - {num_gen} Generations - Total tardiness: {gbest_value}')
