...
def coef_times_velocity(arc_sol_velocity_dict):
    # Randomly generate a coefficient for each item's velocity dictionary
    # and enforce a maximum value of 1
    return [
        {arc: min(1, random.uniform(0, 1) * velocity) for arc, velocity in item_dict.items()}
        for item_dict in arc_sol_velocity_dict
    ]


def coef_times_position_with_gaussian(arc_first, arc_second):
...
def create_cut_set(added_velocity_dict):
    # Generate a single random alpha value for the whole function call
...
def mayfly(name_path_input, num_gen, pop_size, *parameters):
    ...
    for gen in range(num_gen):
        ...
        for sol in range(half_pop_size):
            ...
            mcoef_velocity = coef_times_velocity(male_velocity_dict[sol])
            mpbest_diff = coef_times_position_with_gaussian(male_pbest_arc_sols_cut[sol],male_arc_sols_cut[sol])
            mgbest_diff = coef_times_position_with_gaussian(gbest_arc_sol_cut, male_arc_sols_cut[sol])
            ...
            mcut_set = create_cut_set(mvelocity_check_incon)
        ...
        # ---------------------- Female Mayfly Section ----------------------
            ...
            fcoef_velocity = coef_times_velocity(female_velocity_dict[sol])
            fmale_diff = coef_times_position_with_gaussian(male_arc_sols_cut[sol], female_arc_sols_cut[sol])
            fadded_velocity = add_velocity(fcoef_velocity, fmale_diff)
            fvelocity_check_incon = check_velocity_inconsistency(fadded_velocity)
            fcut_set = create_cut_set(fvelocity_check_incon)
            ...
            extrac_new_male_sols = extract_and_flatten(mevaluation)
            extrac_new_female_sols = extract_and_flatten(fevaluation)
...
gbest_per_gen = mayfly(name_path_input, num_gen, pop_size,seed)
print(gbest_per_gen)